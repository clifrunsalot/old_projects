...
Sub find_and_copy(sht As String)
    ...
    Range(rg).Select
    Rows(ActiveWindow.RangeSelection.row).Select
    ActiveWorkbook.Sheets(sht).Paste
    Application.CutCopyMode = False
...
handle_error:
    ...
    If Len(Trim(commaList)) < 1 Then
    
        msg = "This method cannot be called directly. " & vbCr _
        & "Please use the find_text() method."
        
    Else

        msg = "Recheck search strings and ensure that individual strings " & vbCr _
