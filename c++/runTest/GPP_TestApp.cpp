...
extern "C" {
int runTestGPPMain(int argc, char* argv[ ])
{
   ...
   for (int xInit=0 ; xInit<argc; xInit++)
   {
    ...
    // start of code
    // -------------------------------------------------------------
    cout << "GPP_TestAppMain Executing" << endl;
    ...
    if (argc & 0x01)
    {
      ...
      for (count = 1; count < argc ; count +=2)
      {
        ...
        {   
           cout << "GPP_TestApp Exec Param ID = " << argv[count] << " Exec Parm value = " << argv[count+1] << endl;  
        ...
        else if (!strcmp(argv[count], "NAMING_CONTEXT_IOR"))
        {
          ...
          {  
            ...
            {
              cout << "GPP_TestAppMain ERROR Naming Context IOR narrow to NamingContext failed" << endl;  
            }
          }
          else  // obj is nil
          {
               cout << "GPP_TestAppMain ERROR Naming Context IOR when converted to object is nil" << endl;  
          }   
      ...
      {
        cout << "GPP_TestAppMain NAMING_CONTEXT_IOR ID was NOT found!" << endl;
      }
   
      if (!nameBindingFound)
      {
        cout << "GPP_TestAppMain NAME_BINDING ID was NOT found!" << endl;
      }

      if (!waveformCapabilityFound)
      {
        cout << "GPP_TestAppMain WF Capability was NOT found!" << endl;
      }
